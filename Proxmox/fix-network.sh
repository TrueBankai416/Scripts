...
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --no-disable-offloading    Do not disable network offloading features"
            ...
            echo "  $0 --no-disable-offloading eth0  # Restart without disabling offloading"
            echo "  $0 --keep-offloading       # Auto-detect interface, keep offloading"
            echo ""
            echo "Note: By default, the script disables network offloading features (GSO, GRO, TSO, etc.)"
            echo "      as a workaround for Intel e1000e hardware hang issues. Use --no-disable-offloading"
            echo "      to skip this step if you prefer to keep offloading enabled."
            exit 0
            ;;
        ...
        *)
            if [[ -z "$INTERFACE" ]]; then
                ...
                echo "Error: Multiple interfaces specified"
                echo "Use --help for usage information"
                exit 1
            fi
            shift
...
hardware_reset_interface() {
    ...
    if [[ "$driver" == "e1000e" ]]; then
        ...
        if [[ "$DISABLE_OFFLOADING" == true ]]; then
            ...
            echo -e "${YELLOW}Step 3: Skipping offloading feature disabling (--no-disable-offloading specified)${NC}"
            log_message "INFO" "Skipping offloading feature disabling for $interface due to --no-disable-offloading flag"
        fi
    ...
    if [[ "$driver" != "e1000e" && -n "$driver" ]]; then
        ...
        if [[ "$driver" =~ ^(e1000|igb|ixgbe|i40e).*$ ]]; then
            if [[ "$DISABLE_OFFLOADING" == true ]]; then
                ...
                echo -e "${YELLOW}Intel-based driver detected, but skipping feature workaround (--no-disable-offloading specified)${NC}"
                log_message "INFO" "Skipping offloading feature disabling for Intel driver $driver due to --no-disable-offloading flag"
            fi
            reset_success=true
        fi
    fi
...
main() {
    ...
    log_message "INFO" "Script started with offloading disabling: $DISABLE_OFFLOADING"
